...
export const NODES: PlaneNode[] = [
  ...
  },
  {
    ...
    blurb:
      "Valve-industry datasheets have merged cells, rotated headers, several table schemas on one page — the closed models were extracting them wrong. I curated the image→JSON dataset automatically (Sarvam OCR, a frontier-model pass, then human review) and fine-tuned Qwen3-VL-8B — a vision model that reads the datasheet image directly — on one A100 80GB: LoRA (r=32) SFT with TRL, then GRPO — 8 candidates per image, scored by reward functions I wrote myself: JSON validity, schema compliance, per-field accuracy, a hallucination penalty. Scoring every token equally is the wrong objective — braces and colons are easy; keys and values are the information, so they carry the score. Past the closed models after SFT; GRPO pushed field accuracy further. Everything in W&B; served quantised (AWQ).",
